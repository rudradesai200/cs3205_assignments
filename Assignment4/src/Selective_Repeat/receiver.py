...
class Receiver:
    def __init__(self, args):
        ...
        self.wndw_start = 1
        self.check_acks = [False]*self.args.window_size
    ...
    def send_ack(self, seq_num, attempts):
        if(seq_num < self.wndw_start):
            return
        elif((seq_num < self.wndw_start + self.args.window_size)):
            ...
            if(accept):
                ...
                packet = f"ACK {seq_num},{attempts}"
                if(seq_num != self.wndw_start):
                    packet = f"NAK {self.wndw_start}," + packet

                sock.sendall(str.encode(packet, encoding="utf-8"))
                sock.close()
                self.debug(
                    2, f"Sent {blue('ACK')} for packet with sequence number {seq_num}")

                if(seq_num != self.wndw_start):
                    self.debug(
                        2, f"Sent {red('NAK')} for packet with sequence number {self.wndw_start}")

                self.check_acks[seq_num - self.wndw_start] = True
                while(self.check_acks[0]):
                    self.check_acks.pop(0)
                    self.wndw_start += 1
                    self.check_acks.append(False)
    ...
    def receive_msg(self):
        ...
        while True:
            ...
            if(seq_num.isnumeric() and attempts.isnumeric()):
                self.send_ack(int(seq_num), int(attempts))
            else:
                self.debug(0, "Incorrect message received. Ignoring...")
                continue

            if(self.wndw_start > self.args.max_packs):
                self.debug(
...
def parse_args():
    ...
    parser.add_argument('-sip', '--sender_ip', default="localhost", type=str,
