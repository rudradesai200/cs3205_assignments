    ...
    for pack_len in [256, 512, 1024]:
        for drop_prob in [10**-3, 10**-2, 0.1, 0.3]:
            ...
            p1 = subprocess.Popen(['python', 'receiver.py', '-sip', args.sender_ip, '-sp', str(args.send_port),
                                   '-rp', str(args.recv_port), '-n', str(args.max_packs), '-e', str(drop_prob), '-d', str(args.debug), '-l', str(args.log_level), '-W', str(args.window_size)])
            p2 = subprocess.Popen(['python', 'sender.py', '-rip', args.recv_ip, '-sp', str(args.send_port),
                                   '-rp', str(args.recv_port), '-N', str(
                args.max_packs), '-B', str(args.buffer_size), '-R', str(pack_gen_rate), '-L', str(pack_len),
                '-W', str(args.window_size), '-d', str(args.debug), '-l', str(args.log_level), '-o', str('out.txt')])
...
df.to_csv('../../results/SR_results.csv', index=False)
os.remove('out.txt')
